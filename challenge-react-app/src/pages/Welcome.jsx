...
export default function WelcomePage() {
  ...
  const cityYPosition = useTransform(scrollY, [100, 200], [0, -100]);
  const headerPosition = useTransform(scrollY, [100, 150, 200, 300], [0, 50, 50, 300]);
  const headerSize = useTransform(scrollY, [100, 150, 200, 300], [1, 1.2, 1.4, 1.6])
  const heroOpacity = useTransform(scrollY, [100, 300, 500], [1, 1, 0]);
  const heroYPosition = useTransform(scrollY, [100, 200], [0, -150]);

  return (
    <>
      <header id="welcome-header">
        <motion.div style={{y: headerPosition, scale: headerSize}} id="welcome-header-content">
        ...
        <motion.img
          style={{opacity: cityOpacity, y: cityYPosition}}
          src={cityImg}
          alt="A city skyline touched by sunlight"
          id="city-image"
        />
        <motion.img src={heroImg} style={{y: heroYPosition, opacity: heroOpacity}} alt="A superhero wearing a cape" id="hero-image" />
      </header>
